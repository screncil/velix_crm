          ...
          >
            ...
            >
              ...
              <div class="text-gray-500">
                <div class="mt-3">
                  <label v-if="!fio_input_err" for="name" class="text-gray-500 text-sm">ПІБ</label>
                  <label v-if="fio_input_err" for="name" class="text-red-500 text-sm">Введіть ПІБ більше за 10 символів!</label>
                  <input @change="handleButton" v-model="fio" type="text" id="name" :class="[fio_input_err ? 'outline-none border border-1 border-red-600 mt-2 text-heading text-sm rounded-md block w-full px-3 py-2.5 shadow-md shadow-red-600/30 placeholder:text-body ' : 'outline-none border border-1 border-gray-500 mt-2 text-heading text-sm rounded-md block w-full px-3 py-2.5 shadow-md placeholder:text-body focus:shadow-orange-500/30 focus:border-orange-500']" placeholder="Артем Злючий" required />
                  </div>
                <div class="mt-2">
                  <label v-if="!number_input_err" for="model" class="text-gray-500 text-sm">Номер телефону</label>
                  <label v-if="number_input_err" for="model" class="text-red-500 text-sm">Введіть коректний номер телефону!</label>
                  <input @change="handleButton" v-model="number_phone" type="text" id="model" :class="[number_input_err ? 'border border-1 border-red-600 mt-2 text-heading text-sm rounded-md block w-full px-3 py-2.5 shadow-md shadow-red-600/30 placeholder:text-body focus:border-orange-500 outline-none' : 'border border-1 border-gray-500 mt-2 text-heading text-sm rounded-md block w-full px-3 py-2.5 shadow-md placeholder:text-body focus:shadow-orange-500/30 focus:border-orange-500 outline-none']" placeholder="+380001234567" required />
                </div>
                <div class="mt-2">
                    <label for="model" class="text-gray-500 text-sm">Звідки дізнався</label>
                    <Listbox v-model="selectedWhereKnow">
                        <div class="relative mt-1">
                            ...
                            >
                            <span class="block truncate">{{ selectedWhereKnow.name }}</span>
                            <span
                            ...
                            >
                                ...
                                v-for="where_know in whereknows"
                                :key="where_know.id"
                                :value="where_know"
                                as="template"
                                >
...
import { allWhereKnow } from '../../api/whereFind'


const isOpen = ref(false)

const selectedWhereKnow = ref('')
...
async function openModal() {
  const where = await allWhereKnow()
  whereknows = where;
  isOpen.value = true
}
